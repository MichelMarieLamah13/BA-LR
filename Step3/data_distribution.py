...
def partition_gender_plot(dict_ba, ba, chaine):
    ...
    for bar1, bar2 in zip(plots1.patches, plots2.patches):
        plots1.annotate(format(bar1.get_height(), '.2f'), (bar1.get_x() + bar1.get_width() / 2, bar1.get_height()),
                        ha='center', va='center', size=6, xytext=(0, 5), textcoords='offset points')
        plots2.annotate(format(bar2.get_height(), '.2f'), (bar2.get_x() + bar2.get_width() / 2, bar2.get_height()),
                        ha='center', va='center', size=6, xytext=(0, 5), textcoords='offset points')
...
def loc_gendre(X, meta_vox2):
    ...
    for idx, row in X.iterrows():
        ...
        gendre.append(
            meta_vox2[meta_vox2["VoxCeleb2 ID "] == X["name"][idx].split("/")[-3] + " "]["Gender "].values[0].rstrip())

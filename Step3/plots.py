...
def plot_family_bars(df_plot, ba):
    ...
    p = figure(width=1200, height=600, title="Contribution of variables grouped by families",
               ...
               tooltips=[("Cont_Member", "@Cont_Member_mean"), ("Cont_Family", "@Cont_Family_mean")])
    p.vbar(x='Family_Members', top='Cont_Member_mean', width=1, source=group, line_color="white",
...
def plot_bar_BA(df_cont_bas):
    ...
    index_cmap = factor_cmap('Family_BA', palette=inferno(len(df_cont_bas.BA.unique())),
                             factors=df_cont_bas.BA.unique(), start=1, end=2)
